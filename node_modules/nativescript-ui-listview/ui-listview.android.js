...
function ensureListViewAdapter() {
    ...
    var ListViewAdapter = /** @class */ (function (_super) {
        ...
        ListViewAdapter.prototype.onCreateSwipeContentHolder = function (parent) {
            // let swipeView = builder.parse(this.ownerLv.itemSwipeTemplate);
...
function ensureListViewDataSourceAdapter() {
    ...
    var ListViewDataSourceAdapter = /** @class */ (function (_super) {
        ...
        ListViewDataSourceAdapter.prototype.onCreateGroupViewHolder = function (parent, viewType) {
            if (this.ownerLv.enableCollapsibleGroups) {
                return _super.prototype.onCreateGroupViewHolder.call(this, parent, viewType);
            }
            var nsGroupView = this.ownerLv.getViewForViewType(listViewCommonModule.ListViewViewTypes.GroupView);
            if (!nsGroupView) {
                return _super.prototype.onCreateGroupViewHolder.call(this, parent, viewType);
            }
            var layoutParams = this.ownerLv._getViewLayoutParams();
            if (nsGroupView) {
                this.ownerLv._addView(nsGroupView);
        ...
        ListViewDataSourceAdapter.prototype.onBindGroupViewHolder = function (holder, groupKey) {
            if (this.ownerLv.enableCollapsibleGroups || !holder['nsView']) {
            ...
            if (_super.prototype.isGroupHeader.call(this, position)) {
                ...
                if (this.ownerLv.itemTemplateSelector) {
                    ...
                    if (selector) {
                        ...
                        var dataItem = this.ownerLv.getItemAtIndex(nativeIndex);
                        if (dataItem) {
                            var selectorType = selector(dataItem, nativeIndex, this.ownerLv.items);
                            if (!this.templateTypeNumberString.has(selectorType)) {
                                this.templateTypeNumberString.set(selectorType, this._currentNativeItemType);
                                this._currentNativeItemType++;
                            }
...
var RadListView = /** @class */ (function (_super) {
    ...
    RadListView.prototype._updateHeaderFooter = function () {
        ...
        if (headerView) {
            // let headerView = builder.parse(this.headerItemTemplate, this);
        ...
        if (footerView) {
            // let footerView = builder.parse(this.footerItemTemplate, this);
            ...
            if (!this._swipeActionsBehavior) {
                ...
                var impl = {
                    ...
                    onSwipeStarted: function (event) {
                        var swipeView = that_1.get()._listViewAdapter.getSwipeViewForItem(that_1.get().getItemAtIndex(event.swipedItemPosition()));
                        var mainView = that_1.get()._listViewAdapter.getViewForItem(that_1.get().getItemAtIndex(event.swipedItemPosition()));
                        var args = {
                    ...
                    onSwipeProgressChanged: function (event) {
                        ...
                        var args = {
                            ...
                            index: event.swipedItemPosition(),
                            data: {
                                x: that_1.get().listViewLayout.scrollDirection === "Vertical" ? event.currentOffset() : 0,
                                y: that_1.get().listViewLayout.scrollDirection === "Vertical" ? 0 : event.currentOffset(),
                    ...
                    onExecuteFinished: function (event) {
                        ...
                        var args = {
                            ...
                            index: event.swipedItemPosition(),
                            data: {
                                x: that_1.get().listViewLayout.scrollDirection === "Vertical" ? event.swipePositionWhenReleased() : 0,
                                y: that_1.get().listViewLayout.scrollDirection === "Vertical" ? 0 : event.swipePositionWhenReleased(),
                                swipeLimits: this.swipeLimits
                            }
                        };
                        that_1.get().notify(args);
        ...
        if (!this._loadOnDemandBehavior) {
            ...
            this._loadOnDemandBehavior.addListener(new com.telerik.widget.list.LoadOnDemandBehavior.LoadOnDemandListener({
                onLoadStarted: function () {
                    ...
                    if (!args.returnValue) {
                        that_4.get()._loadOnDemandBehavior.setEnabled(false);
                    }
                },
        ...
        if (!this._selectionBehavior) {
            ...
            this._selectionBehavior.addListener(new com.telerik.widget.list.SelectionBehavior.SelectionChangedListener({
                ...
                onItemIsSelectedChanged: function (position, newValue) {
                    ...
                    if (listView.isDataOperationsEnabled) {
                        // using the ListViewDataSourceAdapter that knows about grouping and filtering to get the real item index in the data source
                        isGroupHeader = listView._listViewAdapter.isGroupHeader(position);
                        position = listView._listViewAdapter.getItemId(position);
                    }
                    if (!isGroupHeader) {
                        var item = listView.getItemAtIndex(position);
